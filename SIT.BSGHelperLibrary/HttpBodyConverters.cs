        ...
        public static async Task CompressStringIntoResponseBody(string stringToConvert, HttpRequest request, HttpResponse response)
        {
            ...
            if (!string.IsNullOrEmpty(stringToConvert))
            {
                if (request.Headers.AcceptEncoding == "deflate, gzip" || request.Headers.AcceptEncoding == "deflate")
                {
                    ...
                    //byte[] convertedString = Encoding.UTF8.GetBytes(stringToConvert);
                    //using (DeflateStream deflateStream = new DeflateStream(new MemoryStream(bytes), CompressionLevel.SmallestSize))
                    //{
                    //    deflateStream.Write(convertedString, 0, convertedString.Length);
